...
$(function(){
	...
	$(window).scroll(function(){
        ...
        if(scrollHeight>0){
        	$(".order-discount").addClass("cartFixed-affirm");
        }
	...
	$("#updateAddress").click(function(){
		$(".__addressPop ").slideUp();
		$("#J_manage_address").slideDown();
		manageAddress();
	});
	...
	$("#addOtherAddress").click(function(){
		$("#frm1")[0].reset();//初始化表单
		$(".__addressPop").slideDown();
		$(".confirm_btn_add").data("id","0");
		$("#J_manage_address").slideUp();
	});
	
	
	//生成订单按钮
	$(".toCreateOrder").click(function(){
		var redisCartItemKeys=[ ];
		var   addressid=$(".check").attr("id");
		$(".mbshopBox ").each(function(){
			redisCartItemKeys.push($(this).attr('id'));
		});
		
		var   content="<form style='display:none' id='topayfor'  action='"+contentPath+"/ordersController/submitOrders' method='post'>";
				content+="<input name='addid' value= '"+addressid+"'/>";
				content+="<input name='redisCartItemKeys' value= '"+redisCartItemKeys+"'/>";
				content+="</form>";
				$(this).append(content);
...
function initAddress(){
	...
	$.ajax({
		dataType:"json",
		url:contentPath+"/AddressInfoController/getAddress",
		async:true,
		type:"POST",   //请求方式
		success:function(res){
			if(res.success){
				$(".order-address-checkbox").text("");
				for(var i=0;i<res.obj.length;i++){ 
					var content2="<li class='default check' id="+res.obj[i].id+">";
					content2+="<span class='postcode' style='display:none'>"+res.obj[i].postcode+"</span>";
					content2+="<p><span class='consignee'>"+res.obj[i].name+"</span><span class='phonenumber'>"+res.obj[i].mobilenumber1+"</span></p>";
					content2+="<span class='addAdress'><i class='province'>"+res.obj[i].province+"</i><i class='city'>"+res.obj[i].city+"</i><i class='area'>"+res.obj[i].district+"</i><i class='street'>"+res.obj[i].address+"</i></span>";
					content2+="<span class='update' onclick='update(this)'>修改</span>";
					content2+="</li>";
					$(".order-address-checkbox").prepend(content2);
				}
						
					$(".order-address-checkbox li").mouseover(function(){
						$(this).addClass("show").siblings().removeClass("show");
						$(this).find(".update").css("display","block");
					});
					
					$(".order-address-checkbox li").mouseout(function(){
						$(this).removeClass("show");
						$(this).find(".update").css("display","none");
					});
		
					$(".order-address-checkbox li").click(function(){
						$(this).addClass("check").siblings().removeClass("check");
					});
				
					$(".order-address-checkbox li").each(function(i){
						$(this).removeClass("check");
					});
				
				$(".order-address-checkbox li").eq(0).addClass("check");
					if($(".order-address-checkbox li").length<=0){
						$(".__addressPop").slideDown();
					}else{
						$(".__addressPop").css("display","none");
					}
...
function manageAddress(){
	$.ajax({
        ...
        url:contentPath+"/AddressInfoController/getAddress",
        async:true,
        data:{"province":$('.province3 option:selected').text(), "city":$('.city3 option:selected').text(),"district" : $('.area3 option:selected').text() , "address" : $('.J_street').val() , "postcode" : $(".J_postcode ").val() ,"name" : $(".J_name").val() ,"mobilenumber1": $(".J_mobile").val() },    //参数值
        type:"POST",   //请求方式
        success:function(res){
            if(res.success){
            	$("#J_manage_address").text("");
                for(var i=0;i<res.obj.length;i++){ 
	              var content2="<ul><li class='default check' id="+res.obj[i].id+">";
		    			content2+="<span class='consignee'>"+res.obj[i].name+"</span>";
		    			content2+="<span class='postcode' style='display:none'>"+res.obj[i].postcode+"</span>";
		    			content2+="<span class='addAdress'><i class='province'>"+res.obj[i].province+"</i><i class='city'>"+res.obj[i].city+"</i><i class='area'>"+res.obj[i].district+"</i><i class='street'>"+res.obj[i].address+"</i></span>";
		    			content2+="<span class='phonenumber'>"+res.obj[i].mobilenumber1+"</span>";
		    			content2+="<span class='update' onclick='updateAddress(this)'>修改</span>";
		    			content2+="<span class='delete' onclick='deleteAddress(this)'  id="+res.obj[i].id+">删除</span>";
		    			content2+="</li></ul>";
	    			$("#J_manage_address").prepend(content2);
                }
            }else{
            	alert(res.msg);
            }
...
function deleteAddress(o){
	...
	$.ajax({
        dataType:"json",
        url:contentPath+"/AddressInfoController/deleteAddress",
        async:true,
        data:{"id":$(o).attr("id") },    //参数值
        type:"POST",   //请求方式
        success:function(res){
            if(res.success){
            	manageAddress();
...
function update(o){
	$(".__addressPop").slideDown();
	$('.province3 option:selected').text($(o).siblings().children(".province").text());
	$('.city3 option:selected').text($(o).siblings().children(".city").text());
	$('.area3 option:selected').text($(o).siblings().children(".area").text());
	$('.J_street').val($(o).siblings().children(".street").text());
	$(".J_postcode ").val($(o).siblings(".postcode").text());  
	$(".J_name").val($(o).siblings().children(".consignee").text());
	$(".J_mobile").val($(o).siblings().children(".phonenumber").text());
	$(".confirm_btn_add").addClass("confirm_btn_update ");
	$(".confirm_btn_add").attr("id",$(o).parent().attr("id"));
	$("#J_manage_address").slideUp();
}
...
function confirm(o){
			...
			//地址校验
			if($('.province3 option:selected').text()=="--请选择省份--"){
				$(".province_error").text("请选择省份！").css("visibility","visible");

				return false;
			}else{
				$(".province_error").text("").css("visibility","hidden");
			}
			//邮编校验
			if($('.J_postcode').val()==""){
				$(".J_postcode_error").text("邮编不能为空！").css("visibility","visible");

				return false;
			}else{
				$(".J_postcode_error").text("").css("visibility","hidden");
			}
			//街道地址校验
			if($('.J_street').val()==""){
				$(".J_street_error").text("街道地址不能为空！").css("visibility","visible");

				return false;
			}else{
				$(".J_street_error").text("").css("visibility","hidden");
			}
			//收货人校验
			if($('.J_name').val()==""){
				$(".J_name_error").text("收货人不能为空！").css("visibility","visible");

				return false;
			}else{
				$(".J_name_error").text("").css("visibility","hidden");
			}
			//手机号码校验
			if($('.J_mobile').val() == ""){
				$(".phone_error").text("手机号不能为空！").css("visibility","visible");

				return false;
			}
			else{
				var re1 = /^1\d{10}$/;
				if (re1.test($('.J_mobile').val())) {
					$(".phone_error").text("").css("visibility","hidden");
				} else {
					$(".phone_error").text("手机号格式错误！").css("visibility","visible");
					return false;
				}
			}
			
			if($(o).hasClass("confirm_btn_update")){
				$.ajax({
					dataType:"json",
					url:contentPath+"/AddressInfoController/updateAddress",
					async:true,
					data:{"id":$('.check').attr('id'),"province":$('.province3 option:selected').text(), "city":$('.city3 option:selected').text(),"district" : $('.area3 option:selected').text() , "address" : $('.J_street').val() , "postcode" : $(".J_postcode ").val() ,"name" : $(".J_name").val() ,"mobilenumber1": $(".J_mobile").val() },    //参数值
					type:"POST",   //请求方式
					success:function(res){
						if(res.success){
							initAddress();	
							...
							$(".order-address-checkbox #"+$(o).attr("id")).addClass("check");
				...
				$.ajax({
					dataType:"json",
					url:contentPath+"/AddressInfoController/addAddress",
					async:true,
					data:{"province":$('.province3 option:selected').text(), "city":$('.city3 option:selected').text(),"district" : $('.area3 option:selected').text() , "address" : $('.J_street').val() , "postcode" : $(".J_postcode ").val() ,"name" : $(".J_name").val() ,"mobilenumber1": $(".J_mobile").val() },    //参数值
					type:"POST",   //请求方式
					success:function(res){
...
function oxj(){
	//小计
	   ...
	   var sum1=0;
	   var sum2=0;
	   $(".xxj").each(function(){ //遍历订单号
		   sum1=0;//获得每个订单
		   $(this).find(".mbshopBox").each(function(){//遍历每款
			       xj=0;//获得每款
			   $(this).find(".xiq").find(".fj").each(function(){//遍历每个尺码
				   je=parseInt($(this).find(".xijsi").children().text());
				   xj=xj+je;
			   });
			   sum1=sum1+xj;
		   });
		   sum2=sum2+sum1;
		   $(this).find(".otder").children().children(".status-wait-buyer-action").text("￥"+sum1);
	   });
	   $(".totalPrice").children().text(sum2);
	   
	   //生成订单编号
	  /* $(".otderRght span").each(function(index){
		   $(this).text("订单"+(index+1));
	   });*/
}
